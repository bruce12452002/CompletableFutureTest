...
//        cf.exceptionallyTest();
    ...
    private void initAsync() {
        ...
        // 包起來的 CompletableFuture.runAsync 是另外一個執行緒，通常執會執行 start 後 end，最後才是裡面的方法
    }
...
//            int i = 1 / 0;
            return id;
        }, ES).whenComplete((res, ex) -> { // whenComplete 使用上一個執行緒；whenCompleteAsync 用新的執行緒
...
//            int i = 1 / 0;
            return id;
        }, ES).whenCompleteAsync((res, ex) -> { // whenComplete 使用上一個執行緒；whenCompleteAsync 用新的執行緒
            System.out.println("內部結果為=" + res);
            System.out.println("例外=" + ex);
    ...
    private void handleTest() throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<Long> rtn = CompletableFuture.supplyAsync(() -> {
            ...
            int i = 1 / 0;
            return id;
        }, ES).handle((res, ex) -> { // handle 使用上一個執行緒；handleAsync 用新的執行緒
            System.out.println("內部結果為=" + res);
            System.out.println("例外=" + ex);
            return 999L;
        });
